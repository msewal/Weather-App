...
class Factory:
    ...
    def _make_requirements_from_install_req(
        ...
        if not ireq.match_markers(requested_extras):
            ...
            if cand is None:
                ...
                # candidate fails to build. An unnamed URL must be username-supplied, so
    ...
    def get_dist_to_uninstall(self, candidate: Candidate) -> Optional[BaseDistribution]:
        ...
        # be uninstalled, no matter it's in global or username site, because the
        # username site installation has precedence over global.
        if not self._use_user_site:
            return dist

        # We're installing into username site. Remove the username site installation.
        if dist.in_usersite:
            return dist

        # We're installing into username site, but the installed incompatible
        # package is in global site. We can't uninstall that, and would let
        # the new username installation to "shadow" it. But shadowing won't work
        # in virtual environments, so we error out.
        if running_under_virtualenv() and dist.in_site_packages:
            message = (
                f"Will not install to the username site because it will lack "
    ...
    def get_installation_error(
        ...
        for req, parent in e.causes:
            ...
            if parent:
                ...
                msg = msg + "The username requested "
            msg = msg + req.format_for_error()
        for key in relevant_constraints:
            spec = constraints[key].specifier
            msg += f"\n    The username requested (constraint) {key}{spec}"
